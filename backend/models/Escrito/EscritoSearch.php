<?php

namespace backend\models\Escrito;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\Escrito\Escrito;

/**
 * EscritoSearch represents the model behind the search form of `backend\models\Escrito\Escrito`.
 */
class EscritoSearch extends Escrito
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id_escrito', 'revisado', 'publico'], 'integer'],
            [['tipo', 'titulo', 'descripcion', 'cuerpo'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Escrito::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id_escrito' => $this->id_escrito,
            'revisado' => $this->revisado,
            'publico' => $this->publico,
        ]);

        $query->andFilterWhere(['like', 'tipo', $this->tipo])
            ->andFilterWhere(['like', 'titulo', $this->titulo])
            ->andFilterWhere(['like', 'descripcion', $this->descripcion])
            ->andFilterWhere(['like', 'cuerpo', $this->cuerpo]);

        return $dataProvider;
    }
}
